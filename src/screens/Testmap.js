...
import ProviderLocationUpdater from './ProviderLocationUpdater';
...
const acceptOffer = async (requestId, offer, request) => {
  try {
    ...
    // Get customer & provider location
    const customerLocation = request?.location;
    if (!customerLocation) return Alert.alert("Error", "Customer location missing!");

    const [lat1, lng1] = customerLocation.split(',').map(Number);

    // Pass provider initial location (or start at customer if unavailable)
    const providerLocation = offer?.providerLocation || customerLocation;
    const [lat2, lng2] = providerLocation.split(',').map(Number);
     await AsyncStorage.setItem('active_request', JSON.stringify({ lat1,
      lng1,
      lat2, 
      lng2,
